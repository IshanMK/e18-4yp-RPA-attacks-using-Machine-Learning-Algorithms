...
def custom_cnn(classes, number_of_samples):
    tf.keras.backend.clear_session()
    tf.random.set_seed(42)
    model = Sequential(name="cnn_lstm")
    model.add(Conv1D(filters=8, kernel_size=10, strides=1, activation='relu', padding='valid', input_shape=(number_of_samples, 1)))
    model.add(Dense(32, activation='relu', kernel_regularizer = l2(1e-4) , kernel_initializer='random_uniform', bias_initializer='zeros'))
    model.add(LSTM(16, activation="sigmoid", return_sequences=True , kernel_regularizer = l2(1e-4)))
    model.add(Flatten())
    model.add(Dense(128, activation='relu', kernel_regularizer = l2(1e-4) , kernel_initializer='random_uniform', bias_initializer='zeros'))
